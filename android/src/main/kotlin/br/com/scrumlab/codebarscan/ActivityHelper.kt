...
class ActivityHelper(private var applicationContext: Context?,
    ...
    companion object {
        const val TAG = "CodebarScanPlugin"
    ...
    fun showScannerActivity(result: MethodChannel.Result, config: Protos.Configuration) {
        if (activity == null) {
            Log.d(TAG, "Could not launch CodebarScannerActivity because the plugin is not attached to any activity")
            return
        }

        activityResultMap[REQ_START_SCAN] = ScanResultHandler(result)

        val intent = Intent(applicationContext, CodebarScannerActivity::class.java)
        intent.putExtra(CodebarScannerActivity.EXTRA_CONFIG, config.toByteArray())
