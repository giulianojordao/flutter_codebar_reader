//
//  CodebarScannerViewController.swift
//  barcode_scan
//
...
import Foundation
import MTBCodebarScanner

class CodebarScannerViewController: UIViewController {
  private var previewView: UIView?
  private var scanRect: ScannerOverlay?
  private var scanner: MTBCodebarScanner?
  ...
  private let formatMap = [
    CodebarFormat.aztec : AVMetadataObject.ObjectType.aztec,
    CodebarFormat.code39 : AVMetadataObject.ObjectType.code39,
    CodebarFormat.code93 : AVMetadataObject.ObjectType.code93,
    CodebarFormat.code128 : AVMetadataObject.ObjectType.code128,
    CodebarFormat.dataMatrix : AVMetadataObject.ObjectType.dataMatrix,
    CodebarFormat.ean8 : AVMetadataObject.ObjectType.ean8,
    CodebarFormat.ean13 : AVMetadataObject.ObjectType.ean13,
    CodebarFormat.interleaved2Of5 : AVMetadataObject.ObjectType.interleaved2of5,
    CodebarFormat.pdf417 : AVMetadataObject.ObjectType.pdf417,
    CodebarFormat.qr : AVMetadataObject.ObjectType.qr,
    CodebarFormat.upce : AVMetadataObject.ObjectType.upce,
  ]
  
  var delegate: CodebarScannerViewControllerDelegate?
  ...
  override func viewDidLoad() {
    ...
    if restrictedBarcodeTypes.isEmpty {
      scanner = MTBCodebarScanner(previewView: previewView)
    } else {
      scanner = MTBCodebarScanner(metadataObjectTypes: restrictedBarcodeTypes,
  ...
  override func viewDidAppear(_ animated: Bool) {
    ...
    MTBCodebarScanner.requestCameraPermission(success: { success in
      if success {
        self.startScan()
      } else {
        #if !targetEnvironment(simulator)
